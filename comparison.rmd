...
```{r}
hepatitis <- read.csv("hepatitis/hepatitis.data", header = FALSE)
hepatitis$V1 <- as.factor(hepatitis$V1)
spect <- rbind(read.csv("spect/SPECT.train", header = FALSE), read.csv("spect/SPECT.test", header = FALSE))
spect$V1 <- as.factor(spect$V1)
pima <- read.csv("pima/pima-indians-diabetes.data", header = FALSE) %>% rename(V1 = V9, V9 = V1)
pima$V1 <- as.factor(pima$V1)
```
